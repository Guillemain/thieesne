...
%   
det_sys = (p2(1) - p1(1))*(p2(2) - p1(2)) - (p3(1) - p1(1))*(p3(2) - p1(2)); % Calcul de alpha

M = zeros(3,3);

Points = [p3(:)';p1(:)';p2(:)']; % On concatène les points pour les calculs 
...
g = [(Points(mod(k+1,3)+1,2) - Points(mod(k+2,3)+1,2));
    (Points(mod(k+2,3)+1,1) - Points(mod(k+2,3)+1,1))];
end