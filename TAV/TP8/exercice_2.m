...
N = 6;
[moy,var_covar] = estimation(I,N,couleurs_classes);
variances = var_covar(:);
[moyennes,ind] = sort(moy);
variances = variances(ind);
...
for i = 1:nb_lignes
	for j = 1:nb_colonnes
		U(i,j) = U(i,j)+regularisation(i,j,k,k(i,j),beta);
		couleurs_pixels(i,j,:) = couleurs_classes(k(i,j),:);
	end
end
figure('Name',['Segmentation par classification'],'Position',[0.6*L,0,0.4*L,0.4*L]);
...
for q = 1:q_max
	for i = 1:nb_lignes
		for j = 1:nb_colonnes
			...
			U_nouv = AD(i,j,k_nouv)+regularisation(i,j,k,k_nouv,beta);
